(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new swiper_core_Swiper(".swiper", {
            ...
            navigation: {
                prevEl: ".swiper__left-arr",
                nextEl: ".swiper__right-arr"
            },
            breakpoints: {
                598.98: {
                    slidesPerView: 2,
    ...
    const products = [{
        ...
        name: "Floral Moss",
        price: 33,
        img: "img/tabs/plant-in-pot1.webp"
    }, {
        id: "plant-2",
        name: "Areca Palm",
        price: 22,
        img: "img/tabs/plant-in-pot2.webp"
    }, {
        id: "plant-3",
        name: "Monstera Obliqua",
        price: 45,
        img: "img/tabs/plant-in-pot3.webp"
    }, {
        id: "plant-4",
        name: "Sansevieria Trifasciata",
        price: 36,
        img: "img/tabs/plant-in-pot4.webp"
    }, {
        id: "plant-5",
        name: "Graptosedum Darley",
        price: 32,
        img: "img/tabs/plant-in-pot5.webp"
    }, {
        id: "plant-6",
        name: "Haworthia Venosa",
        price: 48,
        img: "img/tabs/plant-in-pot6.webp"
    }, {
        id: "plant-7",
        name: "Silver Torch",
        price: 24,
        img: "img/tabs/plant-in-pot7.webp"
    }, {
        id: "plant-8",
        name: "Notocactus Roseoluteus",
        price: 33,
        img: "img/tabs/plant-in-pot8.webp"
    }];
    ...
    function windowLoad() {
        ...
        document.addEventListener("submit", documentActions);
    ...
    function documentActions(e) {
        ...
        if (type === "click") {
            ...
            if (targetElement.closest("a") && html.classList.contains("menu-open")) html.classList.remove("menu-open");
            if (targetElement.closest("summary")) {
                e.preventDefault();
                const spoilerTitle = targetElement.closest("summary");
                const spoiler = spoilerTitle.closest("details");
                const spoilerBody = spoilerTitle.nextElementSibling;
                !spoilerBody.hidden ? spoiler.classList.contains("--active") ? setTimeout(() => {
                    spoilerBody.hidden = true;
                }, 500) : spoilerBody.hidden = true : null;
                !spoiler.open ? spoiler.open = true : setTimeout(() => {
                    spoiler.open = false;
                }, 500);
                app_slideToggle(spoilerBody);
                spoiler.classList.toggle("--active");
            }
            if (targetElement.closest(".tabs__item")) {
                const element = targetElement.closest(".tabs__item");
                const elementId = element.dataset.plantId;
                const currentProduct = products.find(item => String(item.id) === String(elementId));
                if (currentProduct) {
            ...
            if (targetElement.closest(".cart-button")) {
                const cartButton = targetElement.closest(".cart-button");
                cartButton.classList.add("cart-button--hide");
            ...
            if (targetElement.closest(".item-product__delete")) {
                ...
                cart = cart.filter(item => String(item.product.id) !== String(cartItemId));
                localStorage.setItem("cart", JSON.stringify(cart));
                renderCard();
                itemsInCart();
            }
            if (targetElement.closest(".item-product__subtract")) {
                const subtractionButton = targetElement.closest(".item-product__subtract");
                const subtractionItem = subtractionButton.closest(".item-product");
                targetElement.closest(".item-product").id;
                cart.forEach(item => {
                    if (String(item.product.id) === String(subtractionItem.id)) {
            ...
            if (targetElement.closest(".item-product__add")) {
                const subtractionButton = targetElement.closest(".item-product__add");
                const subtractionItem = subtractionButton.closest(".item-product");
                targetElement.closest(".item-product").id;
                cart.forEach(item => {
                    if (String(item.product.id) === String(subtractionItem.id)) item.quantity += 1;
                });
                localStorage.setItem("cart", JSON.stringify(cart));
                renderCard();
                itemsInCart();
        ...
        if (type === "submit") {
            if (targetElement.closest(".popup-form")) {
                e.preventDefault();
                const form = targetElement.closest(".popup-form");
    ...
    function renderCard() {
        ...
        const totalSum = document.querySelector(".content-popup__total-price");
        const hiddenInputsContainer = document.querySelector(".popup-form__hidden-items");
        if (!downCartContent) return;
        if (cart.length === 0) {
            downCartContent.innerHTML = "";
            const emptyCartTemplate = `\n\t\t\t<div class="content-popup__title-empty">Shopping cart is empty</div>\n\t\t\t<div class="content-popup__text">Continue shopping to add product to your cart</div>\n\t\t`;
            downCartContent.insertAdjacentHTML("afterbegin", emptyCartTemplate);
        } else {
            if (!productsConteiner) {
                downCartContent.innerHTML = `\n\t\t\t\t<div class="content-popup__up">\n\t\t\t\t\t\t<h2 class="content-popup__title">Products</h2>\n\t\t\t\t\t\t<div class="content-popup__products products">\n\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class="content-popup__total-price">\x3c!--Total: <span>33,00</span>--\x3e</div>\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class="content-popup__down">\n\t\t\t\t\t\t<form action="https://formspree.io/f/mzdazveg" method="POST" class="popup-form">\n\t\t\t\t\t\t\t<h2 class="content-popup__title">Place an order</h2>\n\t\t\t\t\t\t\t<div class="popup-form__item">\n\t\t\t\t\t\t\t\t<label for="popup-name" class="popup-form__label">Name</label>\n\t\t\t\t\t\t\t\t<input required type="text" name="popup-name" id="popup-name" autocomplete="on"\n\t\t\t\t\t\t\t\t\tclass="popup-form__input" placeholder="Your name">\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class="popup-form__item">\n\t\t\t\t\t\t\t\t<label for="popup-mail" class="popup-form__label">Email</label>\n\t\t\t\t\t\t\t\t<input required type="email" name="popup-mail" id="popup-mail" autocomplete="on"\n\t\t\t\t\t\t\t\t\tclass="popup-form__input" placeholder="Your email address">\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class="popup-form__item">\n\t\t\t\t\t\t\t\t<label for="popup-phone" class="popup-form__label">Phone</label>\n\t\t\t\t\t\t\t\t<input required type="tel" name="popup-phone" id="popup-phone" autocomplete="on"\n\t\t\t\t\t\t\t\t\tclass="popup-form__input" placeholder="Your phone*">\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class="popup-form__item">\n\t\t\t\t\t\t\t\t<label for="popup-text" class="popup-form__label">Comment</label>\n\t\t\t\t\t\t\t\t<textarea name="message" id="popup-text" class="popup-form__input"\n\t\t\t\t\t\t\t\t\tplaceholder="Your comment"></textarea>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class="popup-form__hidden-items">\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<button class="button" type="submit">ORDER</button>\n\t\t\t\t\t\t</form>\n\t\t\t\t\t</div>\n\t\t\t`;
                renderCard();
                return;
            }
            ...
            let finalPrice = 0;
            cart.forEach(el => {
                const productHTML = `\n\t\t\t\t<div class="products__item item-product" id="${el.product.id}">\n\t\t\t\t\t<div class="item-product__img">\n\t\t\t\t\t\t<img src="${el.product.img}" alt="${el.product.id} image">\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class="item-product__content">\n\t\t\t\t\t\t<div class="item-product__name">${el.product.name}</div>\n\t\t\t\t\t\t<div class="item-product__quantity">\n\t\t\t\t\t\t<button class='item-product__subtract'><span></span></button>\n\t\t\t\t\t\t<span class='item-product__number'>${el.quantity}</span>\n\t\t\t\t\t\t<button class='item-product__add'><span></span></button>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class="item-product__price">${el.product.price * el.quantity},00</div>\n\t\t\t\t\t</div>\n\t\t\t\t\t<button class="item-product__delete"></button>\n\t\t\t\t</div>\n\t\t\t`;
                productsConteiner.insertAdjacentHTML("beforeend", productHTML);
                finalPrice += el.product.price * el.quantity;
                const hiddenItem = `\n\t\t\t\t<input type="hidden" name="popup-product" id="popup-product" class="popup-form__input" value="${el.product.id}-${el.product.name}-${el.product.price} * ${el.quantity} = ${el.product.price * el.quantity}">\n\t\t\t`;
                hiddenInputsContainer.insertAdjacentHTML("beforeend", hiddenItem);
            });
            totalSum.textContent = `Total: $${finalPrice},00`;
        }
    }
    renderCard();
    function itemsInCart() {
        const productNumBox = document.querySelector(".cart-button__quantity");
        ...
        if (number === 0) cartButton.classList.add("cart-button--hide"); else if (number > 0 && !popupCart.classList.contains("popup-cart--open")) cartButton.classList.remove("cart-button--hide");
        productNumBox.textContent = number;
    }
    itemsInCart();
    function stableSideBarRem() {
        const arr = document.querySelectorAll("[data-scroll]");
        arr.forEach(el => {
            if (html.classList.contains("menu-open")) {
                let defoltPadding = window.getComputedStyle(el).getPropertyValue("padding-inline-end");
                el.style.paddingInlineEnd = parseInt(defoltPadding) + scrollbarWidth + "px";
    ...
    function tabsChange() {
        document.addEventListener("click", e => {
            ...
            if (!tab) return;
            const tabsWrapper = tab.closest(".tabs");
            if (!tabsWrapper) return;
            const target = tabsWrapper.querySelector(tab.dataset.tabTarget);
            if (!target) return;
            const tabs = tabsWrapper.querySelectorAll("[data-tab-target]");
            const tabContents = tabsWrapper.querySelectorAll("[data-tab-content]");
    ...
    let app_slideToggle = (target, duration = 500) => {
        if (target.hidden) return app_slideDown(target, duration); else return app_slideUp(target, duration);
    };
    window["FLS"] = true;
})();